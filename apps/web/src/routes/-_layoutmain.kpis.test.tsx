...
import { Route } from './_layoutmain.kpis'
...
vi.mock('@/lib/auth', () => ({
  getAccessToken: () => 'tok',
  requireRole: vi.fn().mockReturnValue(() => {}),
}))
...
describe('guard de rota — /kpis', () => {
  it('restringe acesso a gestor e admin', () => {
    // requireRole é chamado durante a inicialização do módulo (createFileRoute)
    expect(vi.mocked(requireRole)).toHaveBeenCalledWith(['gestor', 'admin'])
  })
})
