...
import { Route as RouteIcon, Clock, MapPin, Truck, CheckCircle, Play, Loader2, Sparkles, Trash2 } from 'lucide-react'
...
function RotasPage() {
  ...
  async function removeRota(id: string, nome: string) {
    if (!confirm(`Eliminar a rota "${nome}"? Esta ação não pode ser anulada.`)) return
    try {
      await fetchJson<void>(`/v1/rotas/${id}`, {
        baseUrl: clientEnv.apiBaseUrl,
        headers,
        method: 'DELETE',
      })
      setRotas(prev => prev.filter(r => r.id !== id))
      setRotaSelecionada(prev => (prev?.id === id ? null : prev))
    } catch (err) {
      setListError(getApiErrorMessage(err, 'Não foi possível eliminar a rota.'))
    }
  }
  ...
  return (
    <div className="flex w-full max-w-full flex-col gap-6 pb-12 md:w-[calc(100%_+_(var(--layout-padding)/2))] md:max-w-none">
      ...
      <div className="flex flex-col gap-4">
        <Mapa className="h-[560px] min-h-[420px] w-full rounded-xl overflow-hidden border border-border shadow-sm relative">
          {rotas.map(r => {
          ...
          {rotaSelecionada && (
            rotaSelecionada.paragens.length > 0
              ? rotaSelecionada.paragens.map((p) => (
                  <RotaStopMarker key={p.id} longitude={p.lng} latitude={p.lat} label={p.ordem} ocupacao={p.ocupacao} contentores={p.contentores} fallbackColor={rotaSelecionada.cor}>
              ...
              : rotaSelecionada.waypoints.map(([lat, lng], i) => (
                  <RotaStopMarker key={i} longitude={lng} latitude={lat} label={i + 1} anchor="center" fallbackColor={rotaSelecionada.cor}>
                  ...
                  <div className="mt-2 flex flex-wrap items-center gap-2">
                    {r.estado === 'pendente' && (
                      <button
                        ...
                        className="inline-flex items-center gap-1 rounded-md border border-[var(--primary)]/30 px-2 py-1 text-[11px] font-semibold text-[var(--primary)] transition-all duration-200 hover:-translate-y-0.5 hover:bg-[var(--primary)] hover:text-white hover:shadow-md"
                      >
                    ...
                    {r.estado === 'ativa' && (
                      <button
                        ...
                        className="inline-flex items-center gap-1 rounded-md border border-emerald-500/30 px-2 py-1 text-[11px] font-semibold text-emerald-600 transition-all duration-200 hover:-translate-y-0.5 hover:bg-emerald-600 hover:text-white hover:shadow-md"
                      >
                    ...
                    {isManager && (
                      <button
                        onClick={(e) => { e.stopPropagation(); void removeRota(r.id, r.nome) }}
                        className="inline-flex items-center gap-1 rounded-md border border-destructive/30 px-2 py-1 text-[11px] font-semibold text-destructive transition-all duration-200 hover:-translate-y-0.5 hover:bg-destructive hover:text-white hover:shadow-md"
                      >
