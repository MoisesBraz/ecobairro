  ...
  useEffect(() => {
    ...
    const initializeWebGL = () => {
      ...
      gl = (canvas.getContext("webgl2", params) ||
        ...
        canvas.getContext("experimental-webgl", params)) as WebGLRenderingContext | WebGL2RenderingContext;
    ...
    const compileShader = (type: any, source: any, keywords: string[] | null = null) => {
      if (!gl) return null;
    ...
    const generateColor = () => {
      // Use shades of eco-bairro primary green/teal
      const hue = Math.random() * 0.2 + 0.35; // 0.35 to 0.55 (greens/cyans)
    ...
    const splat = (x: any, y: any, dx: any, dy: any, color: any) => {
      ...
      if (splatProgram.uniforms.aspectRatio && canvas) {
    ...
    const setupEventListeners = () => {
      ...
      // Listen on the container or window? We want the effect only when hovering the parent area.
      // The canvas has pointer-events-none, so we should listen on its parent.
      const parent = canvas.parentElement;
      if (parent) {
        parent.addEventListener("mousedown", handleMouseDown as any);
        parent.addEventListener("mousemove", handleMouseMove as any);
        parent.addEventListener("touchstart", handleTouchStart as any, { passive: false });
        parent.addEventListener("touchmove", handleTouchMove as any, { passive: false });
        parent.addEventListener("touchend", handleTouchEnd as any);
      }

      return () => {
        if (parent) {
          parent.removeEventListener("mousedown", handleMouseDown as any);
          parent.removeEventListener("mousemove", handleMouseMove as any);
          parent.removeEventListener("touchstart", handleTouchStart as any);
